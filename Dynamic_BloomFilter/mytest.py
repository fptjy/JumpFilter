from DynamicBF import Dynamic_BF
import random
import time
...
sys.path.append(r"F:/JumpFilter_Experiment/Jump_CuckooFilter")
from cuckoofilter_JCF import CuckooFilter
...
DBF = Dynamic_BF(n=2 ** 15, m=1024 * 18, k=8, fp_threshold=0.00048)
...
# print(y2)
for i in range(len(DBF.DBF)):
    print(DBF.DBF[i])

print("JpCF.Size", DBF.Size)
...
##DCF_4_test_result
# 1. 创建文件对象
f = open('DBF_test_result_k8_m1024_8.csv', 'w', encoding='utf-8', newline="")
# 2. 基于文件对象构建 csv写入对象
csv_writer = csv.writer(f)
# 4. 写入csv文件内容
for i in range(len(DBF_test_result)):
    csv_writer.writerow([str(DBF_test_result[i])])
# 5. 关闭文件
f.close()
