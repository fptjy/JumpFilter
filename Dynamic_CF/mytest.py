from Dynamic_CuckooFilter import Dynamic_CF
import random
from cuckoofilter_for_DCF import CuckooFilter_DCF
import time
...
##过滤掉到达但未离开的元素
CF_filtrate = CuckooFilter_DCF(capacity=2 ** 21, bucket_size=4, fingerprint_size=20)
...
DCF = Dynamic_CF(capacity=2 ** 15, fpr=0.00048, exp_block_number=8, initial_block_number=1)
...
for i in range(len(insert_result)):
    if insert_result[i] == "non" or len(insert_result[i]) == 0:
        ...
        x2 = len(DCF.DCF) * DCF.single_table_length * 4
        DCF_test_result.append(x2)
...
print(" ")
y2 = len(DCF.DCF) * DCF.single_table_length * 4
print(y2)
for i in range(len(DCF.DCF)):
    print(DCF.DCF[i])

print("DCF.Size", DCF.Size)
...
##DCF_4_test_result
# 1. 创建文件对象
f = open('DCF_test_result_0.8_2.csv', 'w', encoding='utf-8', newline="")
# 2. 基于文件对象构建 csv写入对象
csv_writer = csv.writer(f)
# 4. 写入csv文件内容
for i in range(len(DCF_test_result)):
    csv_writer.writerow([str(DCF_test_result[i])])
# 5. 关闭文件
f.close()
