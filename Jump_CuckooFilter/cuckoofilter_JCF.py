...
import bucket_JCF  # 相对导入from . import bucket这种导入方式会报错
import exceptions
...
class CuckooFilter(object):
    ...
    def __init__(self, capacity, bucket_size=4, fingerprint_size=13,
        ...
        self.buckets = [bucket_JCF.Bucket(size=bucket_size)
    ...
    def _get_index(self, item):
        index = hashutils_DJBhash_JCF.hash_code(item) % self.capacity
        return index

    def _get_alternate_index(self, index, fingerprint):
        alt_index = (index ^ hashutils_DJBhash_JCF.hash_code(fingerprint)) % self.capacity
        return alt_index
    ...
    def insert(self, item):
        ...
        filter is full.
        """
        fingerprint = hashutils_DJBhash_JCF.fingerprint(item, self.fingerprint_size)
