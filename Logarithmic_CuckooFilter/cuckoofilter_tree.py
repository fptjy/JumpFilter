...
import bucket_LDCF  # 相对导入from . import bucket这种导入方式会报错
import exceptions
import hashutils_DJBhash_LDCF


class CuckooFilter_Tree(object):
    """
    Cuckoo Filter class.
    ...
    def __init__(self, level, number, capacity, bucket_size=4, fingerprint_size=13,
        ...
        self.buckets = [bucket_LDCF.Bucket(size=bucket_size)
    ...
    def _get_index(self, item):
        index = hashutils_DJBhash_LDCF.hash_code(item) % self.capacity
        return index

    def _get_alternate_index(self, index, fingerprint):
        alt_index = (index ^ hashutils_DJBhash_LDCF.hash_code(fingerprint)) % self.capacity
        return alt_index
    ...
    def insert(self, item):
        ...
        filter is full.
        """
        fingerprint = hashutils_DJBhash_LDCF.fingerprint(item, self.fingerprint_size)
    ...
    def insert_eviction(self, h, fingerprint):
        """
        Insert an item into the filter.
    ...
    def delete_eviction(self, h, fingerprint):
        """
        Delete an item from the filter.
