# from JumpCF import Jump_CF
import random
from cuckoofilter_tree import CuckooFilter_Tree
import time
from Logarithmic_CF import Logarithmic_DCF
...
##过滤掉到达但未离开的元素
CF_filtrate = CuckooFilter_Tree(level=0, number=0, capacity=2 ** 21, bucket_size=4, fingerprint_size=20)
...
##开始测试
ldcf = Logarithmic_DCF(capacity=2 ** 15, fpr=0.00048, exp_block_number=8)
...
for i in range(len(insert_result)):
    if insert_result[i] == "non" or len(insert_result[i]) == 0:
        ...
        x2 = len(ldcf.LDCF) * ldcf.single_table_length * 4
        LDCF_test_result.append(x2)
...
print(" ")
y2 = len(ldcf.LDCF) * ldcf.single_table_length * 4
print(y2)
for i in range(len(ldcf.LDCF)):
    print(ldcf.LDCF[i])

print("JpCF.Size", ldcf.Size)
print("时间开销：", end - start)
print("LDCF长度：", len(ldcf.LDCF))
...
##DCF_4_test_result
# 1. 创建文件对象
f = open('LDCF_test_result_0.8(new_compact).csv', 'w', encoding='utf-8', newline="")
# 2. 基于文件对象构建 csv写入对象
csv_writer = csv.writer(f)
# 4. 写入csv文件内容
for i in range(len(LDCF_test_result)):
    csv_writer.writerow([str(LDCF_test_result[i])])
# 5. 关闭文件
f.close()
